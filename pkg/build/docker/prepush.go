...
// TODO: Test prepush
// TODO: Getting errors like this for when prepush takes awhile (e.g. 7
// minutes). I think the server's keepalive may be misconfigured. But i get
// these errors on master as well.
// DEBU[2020-08-30T11:11:02-07:00] Prepush failed. Proceeding with a full image push  error="get tag result: rpc error: code = Unavailable desc = transport is closing" service=web
// https://pkg.go.dev/google.golang.org/grpc/keepalive?tab=doc#ServerParameters
func pushBaseImage(dockerClient *docker.Client, blimpToken string, regCreds auth.RegistryCredentials, dockerfilePath, taggedImageName string) error {
	baseImageName, err := getBaseImage(dockerClient, dockerfilePath)
	if err != nil {
