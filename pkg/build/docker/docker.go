...
func (c client) BuildAndPush(serviceName, imageName string, opts build.Options) (digest string, err error) {
	prePushErrChan := make(chan error)
	go func() {
		prePushErrChan <- pushBaseImage(c.client, c.blimpToken, c.regCreds, filepath.Join(opts.Context, opts.Dockerfile), replaceTag(imageName, "base"))
	}()
	...
	// Wait for the prepush to complete. If the prepush isn't complete yet,
	// indiciate that we're waiting for it to finish.
	var prePushErr error
	select {
	case prePushErr = <-prePushErrChan:
	default:
		pp := util.NewProgressPrinter(os.Stdout, "Waiting for base image to be uploaded")
		go pp.Run()
		prePushErr = <-prePushErrChan
		pp.Stop()
	}

	if prePushErr != nil {
		log.WithField("service", serviceName).WithError(prePushErr).Debug("Prepush failed. Proceeding with a full image push")
	}
...
// getDockerClient get a Docker client, and validates that the server will
